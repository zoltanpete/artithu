...
**Polish (Task 011 Part 0)**: owner review of the shipped page approved the direction with one note — the ambient points read slightly too faint next to the core. Desktop and mobile field-mark size/opacity were both increased ~15% (e.g. desktop size `1.4 + n*0.5` → `1.6 + n*0.58`; opacity `0.22 + n*0.4` → `0.26 + n*0.46`), keeping the same per-mark variation and irregular placement, no new marks, no connecting lines, no labels, core geometry and the underlying "ambient field" concept both unchanged. Verified in a real browser at 390px and 1440px.

### Tardify page direction (Task 011)

`/tardify/` is the fourth page with a locked-visual-language decision to make, and the first with **no approved product screenshot at all** (`public/`/`src/assets/` both empty; DOC/13 explicitly gates "Approved product screenshots for ARTIT site" and "Exact approved ARTIT ↔ Tardify origin wording"). DOC/07's own pre-existing Tardify principle ("owned proof of engineering philosophy... a typography-forward treatment... until a screenshot is approved") already anticipated this constraint; Task 011 is where a real page had to honor it.

**Three concepts, built as real prototypes** (`src/pages/art-direction/011-tardify-concepts.astro`) and compared, not reasoned about abstractly:

- **Concept A — Product as Proof.** With no screenshot to show, this reduces to typography plus the homepage's own already-approved "specified plate" Tardify treatment (`.tardify-proof` — Task 007D) extended to page scale.
- **Concept B — Product Lineage / Continuity.** A row of discrete accent ticks along a thin line, representing "continuous development" without literal dates. **Rejected**: a row of discrete marks is itself an implied count/tally — the same failure mode Task 010A's rejected "ascending record bars" had, just without the ascending part. Six ticks reads as "six of something," a claim nothing backs.
- **Concept C — Owned System / Same Standard.** Two bordered boxes ("ÜGYFÉL"/"TARDIFY") joined by a dashed "=" line. **Rejected**: reads as a feature/product comparison device — close enough to the "feature-card grid" and "product-tour" anti-patterns this task's own brief named to avoid, even at just two boxes.

**Selected: Concept A**, realized as **no new diagram** — `SystemMap`/`OperatingFitField`/`LivingSystemField` already cover flow, directional field, and ambient field; this task's own brief explicitly warned against "diagram number four merely to complete a family," and neither B nor C found a genuinely new, safe semantic job a fourth diagram would fill. `/tardify/`'s "same standard" section reuses `.tardify-proof` directly (a second real usage of the exact device, after the homepage's own brief Tardify teaser) rather than inventing new grammar — the restrained plate itself, without an image, is what signals "this is real, owned software," per the already-locked principle.

**Locked addition to the brand-face principle**: not every page needs a *new* visual model even when it needs *a* visual identity — reusing an existing device on a second page, at fuller scale, is a legitimate, often better, answer than inventing a fourth one. Evaluate "does this page need its own model" and "would a new model actually say something the existing three don't" as two separate questions; `/tardify/` answered no to both.

### Tardify brand-face — Product Specimen Plate (Task 011A)

Owner visual review did **not** approve Task 011's shipped page: correct on content, but a repeated heading → paragraph → whitespace → surface-change rhythm with no focal point, and `.tardify-proof` alone too quiet to carry a page-level identity. Task 011A reopened the Hero specifically — not the content architecture, not the CTA strategy, not `.tardify-proof`'s own role downstream.

**Diagnosis, stated before designing**: the desktop Hero's large right-hand territory was unearned empty space; the page was recognizably ARTIT (typography, accent color, restraint) but not recognizably a *product story* — nothing on the page read as an artifact, only as argument about one.

**Product-evidence re-audit**: unchanged from Task 011. `public/` and `src/assets/` remain empty; no approved screenshot exists anywhere in the repository. Whatever imagery is publicly visible on `tardify.hu` is not, by that fact alone, approved for reuse on ARTIT.hu — no repository documentation authorizes copying it, and none was consulted for this task. The exploration proceeded on the honest abstract-specimen path, same as Task 011.

**Three concepts, built as real prototypes** (`src/pages/art-direction/011a-tardify-brand-face-concepts.astro`, each rendered inside a real two-column Hero mock) and judged from the rendered comparison, not from prose:

- **Concept A — Product Specimen Plate.** A technical specification plate: identity band (TARDIFY wordmark) → rule → construction/blueprint-texture band (fine grid, unlabeled tick marks) → rule → quiet signature mark, bounded by a plate that's deliberately open on its right edge (three sides drawn, no closing stroke) rather than a closed card. **Selected.**
- **Concept B — Layered Product Surface.** Two to three offset, sharp-cornered rectangles suggesting one product worked on from multiple angles. **Rejected on the rendered evidence**: even with uniform stroke opacity and no size/fade progression (specifically built to avoid a "Version 1→2→3" reading), the offset planes read too close to stacked browser windows or note cards once actually rendered — the "fake browser" rejection test this task's own brief names.
- **Concept C — Product Fragment / Evidence Window.** A large, mostly-empty cropped grid window with a diagonal line pointing to an off-box "TARDIFY" label. **Rejected on the rendered evidence**: too sparse to solve the actual diagnosed problem (empty Hero territory) — it would have made the emptiness worse, not better — and the pointer line reads as an annotation/callout device, adjacent to the flowchart-arrow reading the brief also warns against.

**Selected: Concept A**, implemented as `TardifySpecimen.astro` (`src/components/tardify/TardifySpecimen.astro`). Genuinely distinct from all three other brand-face devices: `SystemMap`/`OperatingFitField`/`LivingSystemField` are all curve-and-node/radial-field compositions; a bounded rectangular plate is a different geometric family entirely, and cannot be mistaken for any of them. It does not resemble a browser mockup, dashboard, pricing panel, or wireframe — no chrome, no rounded corners, no drop shadow, no fake interactive elements.

**Relationship to `.tardify-proof` — Option 2 (resolved)**: the specimen's corner registration mark reuses `.tardify-proof__mark`'s exact stroke color/width/opacity, making the Hero artifact and the downstream "same standard" plate members of the same product-specific grammar rather than two unrelated devices. The downstream section was left unchanged (checked on the full rendered page, not assumed): it remains a small, quiet echo of the larger Hero artifact, not a duplicate of it, so no redundancy exists and no Option-3 simplification was needed.

**Downstream rhythm**: reviewed on the full rendered page after Hero integration. No further section was decorated — the Hero specimen now supplies the page's one density peak, and the intended contrast (strong focal Hero → quieter argument → restrained proof echo → relationship → close) reads correctly without any other section competing for attention.

**Hero composition**: asymmetric two-column at 900px+, a `5fr/4fr` copy:visual ratio — deliberately not copied from `/egyedi-fejlesztes/`'s `3fr/2fr` or `/munkaink/`'s `2fr/1fr`. The plate's portrait geometry needs vertical room more than extra width, but as the page's one focal artifact (there is no other visual anywhere on this page) it earns closer-to-equal weight than either existing visual gets. Single column below 900px with a separately-composed, simplified mobile plate (closed rectangle, no open edge, fewer bands, no tick marks) — not the desktop specimen scaled down.

**Content discipline**: the one visible word, "TARDIFY," is a hardcoded literal in the component, not YAML content — the product's own proper noun, identical in every locale, not a content decision. No module name, feature label, release label, status badge, technical ID, version string, date, or metric appears anywhere in the geometry.
...
### Kapcsolat art direction (Task 013)

`/kapcsolat/` is the sixth page with a locked-visual-language decision to make, and the first carrying this session's own explicit standing lesson (stated in the task's own Context section): do not ship a structurally correct but visually inert page and defer visual identity to a follow-up task, as happened for `/egyedi-fejlesztes/` (009→009A), `/munkaink/` (010→010A), `/tardify/` (011→011A), and `/rolunk/` (012→012A). The composition had to be intentional in this task itself.

**Page-specific content available for the Hero**: unlike the other five pages, `/kapcsolat/` has real, itemizable content ready-made for a Hero device — the page's own four "what's useful to tell us" prompts (Section 2's content) — not borrowed from elsewhere and not invented for the visual.

**Three concepts, built as real prototypes** (`src/pages/art-direction/013-kapcsolat-concepts.astro`) using the real approved copy, not placeholder text, and judged from the rendered comparison at 1440px:

- **Concept A — Conversation Field.** The four prompts staggered in the desktop right column as short mono lines with a small accent tick each — irregular placement, no connecting lines, no cards, no numbering. **Selected.**
- **Concept B — Editorial Intake.** A large single-column H1 with the four prompts running as a horizontal mono band beneath it. **Rejected on the rendered evidence**: the band is a single thin line of small text — at desktop width this reintroduced almost exactly the functionless dead zone Task 012A had just fixed for `/rolunk/`'s Hero, one task earlier in the same session.
- **Concept C — Minimal Working Note.** The four prompts listed inside a bounded left-margin-rule surface with one small categorical label ("KIINDULÓPONT"). **Rejected on semantic grounds, not rendered failure** — it passed every structural/rejection test (not empty, not a clone, not a fake form) but its registration-mark-and-label framing read slightly more formal/procedural than this specific page's own thesis ("you do not need a polished description") wants to signal. The strongest concept for `/rolunk/`'s tone in Task 012A is not automatically the strongest concept for this page's different tone — evaluated fresh, not by precedent.

**Selected: Concept A**, implemented as the `.kapcsolat-hero__grid`/`.kapcsolat-prompt-field` rules in `foundation.css` (no new component — see "Componentization" below). Genuinely distinct from all four SVG brand-faces (no diagram at all) and from `/rolunk/`'s H1-regrid device (this isn't a headline treatment, it's real list content given a field-like arrangement). `c.prompts` is authored once in YAML and rendered twice — as the Hero's staggered teaser and, after Task 013A, as the Conversation Workspace's full indexed list (originally a separate standalone section, since restructured — see below).

**Locked addition**: when a page has its own ready-made, itemizable content (not borrowed, not invented for the visual), arranging that real content as the Hero's focal device is preferable to either inventing a diagram or leaving typography to carry the desktop canvas alone — a fifth tool alongside flow/field/plate/typographic-regrid, available specifically when the page's own content shape supports it.

### Kapcsolat post-Hero visual pass (Task 013A)

Task 013's Hero was approved outright — locked, not reopened. Owner review found everything *after* it visually conventional: a standalone "Ez segít elindítani a beszélgetést." section that turned out, on the real render, to merely repeat the Hero's own four prompts with no independent job, followed by a form section carrying the exact same functionless-desktop-dead-zone problem Task 012A had already fixed for `/rolunk/`'s Hero — a single narrow bordered-input column against a mostly empty 1440px canvas. Both diagnoses were verified against the actual rendered page before any redesign began, not assumed from the owner's hypothesis alone.

**Three concepts, built as real prototypes** (`src/pages/art-direction/013a-contact-workspace-concepts.astro`) using the real form field markup/classes (not an approximation) and judged from the rendered comparison at 1440px:

- **Concept A — Working Brief / Intake Sheet.** A technical frame (top rule, mono label, small margin ticks beside each field) wrapped around the same narrow single-column form. **Rejected on the rendered evidence**: the frame is a real, restrained addition, but the underlying dead zone is untouched — still roughly two-thirds of the desktop canvas empty. A decorative header on the same weak layout, not a structural fix.
- **Concept B — Conversation → Starting Point.** The four prompts (context, as a real indexed list) and the actual form (the starting point) become one two-column composition, sharing a top rule, no connector between the columns. **Selected.**
- **Concept C — Editorial Form.** No surrounding device at all — large numbered field labels (01/NÉV, 02/EMAIL, 03/HELYZET), rules between fields, an oversized dominant textarea, bold typographic hierarchy carrying the identity. **Rejected on the rendered evidence**: genuinely striking and gave the textarea real presence, but it occupies the same narrow column as A and Task 013's original form — the same desktop dead zone persists, just styled more boldly. It also has nowhere for the four prompts to go, which would have reopened the prompt-duplication question rather than resolving it.

**Selected: Concept B**, implemented as `.kapcsolat-workspace`/`.kapcsolat-workspace__list` in `foundation.css`, replacing both the old standalone prompt section and the old plain form section with one "Conversation Workspace" section. This was not a foregone conclusion — B was the task's own working hypothesis, but A and C were built and genuinely compared, not skipped; B won because it's the only concept that solves the dead zone *and* the prompt duplication in the same move, since the prompts now have exactly one full rendering instead of two. `2fr/3fr` at 900px+: the context column needs less width than the form column, which now also gets a taller, more visually weighted textarea (`.contact-form__field--message`) so the actual question — "what's your situation" — reads as the field that matters, not one of three identical boxes.

**Relationship to `TardifySpecimen` and other brand-faces**: none of the three concepts (including the two rejected) used a closed/open plate, grid texture, or wordmark — Concept A's frame was checked specifically against `TardifySpecimen`'s silhouette during exploration and kept deliberately thinner (one rule + one label + margin ticks, no boundary box) to avoid becoming the same device with different labels, and that device was rejected anyway on other grounds.
