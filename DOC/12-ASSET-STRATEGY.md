...
Favicon/app icons — resolved (Task 019), see below.
...
**Default Open Graph share image — implemented (Task 017)**: `public/og-default.png` (1200×630). Deliberately built only from already-approved material, nothing new: the "ARTIT" wordmark styled identically to `SiteHeader.astro`'s own logo treatment (600 weight, `--color-text`), and the homepage's own already-published SEO tagline ("Egyedi üzleti szoftverfejlesztés senior tapasztalattal." — `home/content.yaml`'s `seo.title`), on the site's own `--color-bg`/`--color-accent` tokens, real Work Sans (self-hosted, same font file as production). Generated by rendering a small static HTML file with those exact tokens through Playwright at exactly 1200×630 and saving the screenshot — not a new illustration or icon, a typographic card assembled from content and tokens that already existed. Wired as `BaseLayout.astro`'s default `ogImage`, so every page gets a real share preview without each page component needing to opt in.

**Favicon — implemented (Task 019)**: `public/favicon.svg` (primary, scalable), `favicon-32x32.png`/`favicon-16x16.png` (PNG fallback for browsers without SVG-favicon support), `apple-touch-icon.png` (180×180, iOS/iPadOS home-screen). The mark: a simple geometric "A," drawn as two stroked line segments (not rendered text — a favicon can't rely on a specific font being available to the browser's own icon rasterizer) in the locked accent color (`#37325c`) on a solid rounded-square background in `--color-bg`'s tone, so it reads identically regardless of the browser tab's own light/dark theme. Not a logo redesign, not a wordmark squeezed into 16px, not a new brand-identity exploration — a single geometric letterform applied to the one format the site genuinely lacked, using colors and proportions already locked by the design system. Verified legible down to the real 16×16 rendered size (not just designed at a larger size and assumed to scale). No `.ico` (the SVG + PNG fallback pair covers every current browser a "modern production site" needs to support) and no PWA manifest (no genuine use case exists for one).
