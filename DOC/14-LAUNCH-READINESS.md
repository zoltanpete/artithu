...
**Update (Task 019) — final state: `READY TO DEPLOY`.** Every item Task 018's update above left open is now closed: Bucket A1 (privacy/legal identification) is implemented, not just fact-supplied — `/adatkezeles/`/`/en/privacy/`, linked from the footer and the contact form. Bucket B4 (production domain) is wired (`astro.config.mjs`'s `site`). Bucket B6 (relative JSON-LD breadcrumb URLs) is resolved — both case pages' `BreadcrumbList` now emits absolute URLs. Bucket B7 (favicon) is resolved. B8 (current-page nav) and B9 (Tardify CTA redundancy) are both resolved. The sitemap this file's own "Prioritized path to launch" anticipated is live (18 URLs). See `13-CONTENT-GAPS-AND-VALIDATION.md`'s Task 019 section and `11-ACCESSIBILITY-AND-PERFORMANCE.md`'s Task 019 entry for the full verification trail, including one real bug this task's own QA found and fixed (a heading-overflow bug on the new privacy page, unrelated to any specific Task 019 scope item, fixed sitewide at the root cause). No further pre-launch implementation task is recommended — see the Task 019 completion report for the full reasoning.
